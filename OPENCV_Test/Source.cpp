...
Mat makeQMatrix_2 (double w, double h)
{
	double f = 0.8*w;
...
void write_ply(std::string fn, Mat verts, Mat colors)
{
	...
	//std::ifstream color_file("Myfiles.txt");
	std::string str;
...
int main(int argc, char** argv)
{
	...
	std::string img3_filename = "D:/opencvcart/aloeL1.jpg";
	std::string intrinsic_filename = "";//"D:/opencvcart/intrinsics.yml";
	std::string extrinsic_filename = "";//"D:/opencvcart/extrinsics.yml";
	std::string disparity_filename = "";//"D:/opencvcart/disparity.dsp";
	...
	cout << "loading..." << endl;
	img1_filename = argv[1];//"C:\opencv\sources\samples\data\left.jpg";//parser.get<std::string>(0);
	img2_filename = argv[2]; //"C:\opencv\sources\samples\data\right.jpg";//parser.get<std::string>(1);
	...
	minDisparity = parser.get<int>("min-disparity");//16;//16; //-64//0
	numberOfDisparities = parser.get<int>("max-disparity");//192-minDisparity;//192;//parser.get<int>("max-disparity");//16
	SADWindowSize = parser.get<int>("blocksize");//5;//5;//parser.get<int>("blocksize"); //1
	scale = parser.get<float>("scale");//0.5;//parser.get<float>("scale"); //0.5
	no_display = parser.has("no-display");//jparser.has("no-display"); //false
	if (parser.has("i"))
	...
	Mat img3 = imread(img3_filename, color_mode);
	Mat color;
	...
	//{
		...
		Q = makeQMatrix_2(img1.cols, img1.rows);

		Mat xyz;
		reprojectImageTo3D(disp, xyz, Q, true);
		cvtColor(img1, color, COLOR_BGR2RGB);
		//cout << xyz;
		//ofstream out("Myfiles_2.txt");
		//out << cv::format(color, 3);
		//out.close();
		//int x = 0;
		//ifstream in("Myfiles.txt", ios::binary | ios::in); //Открываем файл в двоичном режиме только для чтения
